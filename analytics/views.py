...
def website_health(request):
    ...
    production_users = User.objects.exclude(profile__is_test_data=True)
    production_profiles = UserProfile.objects.filter(is_test_data=False)
    users_total = production_users.count()
    active_users = production_users.filter(is_active=True).count()
    staff_users = production_users.filter(is_staff=True).count()
    profiles_total = production_profiles.count()
    missing_profiles = max(users_total - profiles_total, 0)

    production_cvs = CV.objects.exclude(user__profile__is_test_data=True)
    cvs_total = production_cvs.count()
    valid_cvs = production_cvs.filter(is_valid_cv=True).count()
    rejected_cvs = production_cvs.filter(Q(is_valid_cv=False) | Q(validation_status="rejected")).count()
    cvs_without_storage = production_cvs.filter(storage__isnull=True).count()
    production_storage = CVStorage.objects.exclude(user__profile__is_test_data=True)
    storage_total = production_storage.count()
    storage_warning_count = production_storage.filter(used_storage__gte=0.8 * 10 * 1024 * 1024).count()

    production_results = ATSResult.objects.exclude(user__profile__is_test_data=True)
    ats_total = production_results.count()
    ats_completed = production_results.filter(status="completed").count()
    ats_failed = production_results.filter(status="failed").count()
    ats_last_7_days = production_results.filter(created_at__gte=last_7_days).count()
    ats_avg_score = production_results.aggregate(avg=Avg("score"))["avg"] or 0
    low_score_results = production_results.filter(score__lt=50).count()
    results_without_job_role = production_results.filter(job_role__isnull=True).count()

    production_batches = EnterpriseBatch.objects.exclude(user__profile__is_test_data=True)
    production_candidates = EnterpriseCandidateResult.objects.exclude(batch__user__profile__is_test_data=True)
    enterprise_batches = production_batches.count()
    enterprise_candidates = production_candidates.count()
    enterprise_avg_score = production_candidates.aggregate(avg=Avg("score"))["avg"] or 0

    plan_count = SubscriptionPlan.objects.filter(is_active=True).count()
    production_subscriptions = CustomerSubscription.objects.exclude(user__profile__is_test_data=True)
    subscriptions_total = production_subscriptions.count()
    active_subscriptions = production_subscriptions.filter(status="active").count()
    past_due_subscriptions = production_subscriptions.filter(status="past_due").count()
    expired_active_subscriptions = production_subscriptions.filter(
        status="active",
        current_period_end__lt=now,
    ).count()

    production_transactions = PaymentTransaction.objects.exclude(user__profile__is_test_data=True)
    paid_transactions = production_transactions.filter(status="paid")
    transactions_total = production_transactions.count()
    transactions_last_30_days = production_transactions.filter(created_at__gte=last_30_days).count()
    ...
    receipt_failures = production_invoices.exclude(receipt_email_status__in=["sent", "not_sent"]).count()
    receipts_not_sent = production_invoices.filter(status="paid", receipt_sent_at__isnull=True).count()
    ...
    for plan_code, label in plan_labels.items():
        plan_users = production_profiles.filter(plan=plan_code).count()
        active_plan_users = production_profiles.filter(plan=plan_code, user__is_active=True).count()
        plan_validations = production_results.filter(user__profile__plan=plan_code, created_at__gte=last_30_days).count()
        plan_generated = GeneratedCV.objects.filter(user__profile__plan=plan_code, user__profile__is_test_data=False, created_at__gte=last_30_days).count()
        plan_enterprise_batches = production_batches.filter(user__profile__plan=plan_code, created_at__gte=last_30_days).count()
        ...
        plan_refunds = _money(production_refunds.filter(transaction__plan__code=plan_code, status__in=["approved", "processed"]).aggregate(total=Sum("amount"))["total"])
